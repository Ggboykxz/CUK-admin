...
$statsParInstitut = db()->fetchAll("
    SELECT i.sigle, i.nom, COUNT(e.id) as count 
    FROM instituts i 
    LEFT JOIN filieres f ON i.id = f.institut_id 
    LEFT JOIN etudiants e ON f.id = e.filiere_id AND e.annee_academique_id = ? 
    GROUP BY i.id, i.sigle, i.nom
", [$anneeCourante['id'] ?? 0]);

$etudiantsRecents = db()->fetchAll("
...
$absencesRecentes = db()->fetchAll("
    SELECT a.*, e.nom, e.prenom, ec.nom as matiere 
    FROM absences a 
        ...
        <div class="row align-items-center">
            ...
            <div class="col-md-4 text-end">
                <span class="badge bg-success fs-6 px-3 py-2">
                    <i class="bi bi-calendar3"></i> <?= htmlspecialchars($anneeCourante['annee'] ?? '') ?>
                </span>
            </div>
        </div>
    </div>
                    ...
                    <div class="row text-center">
                        ...
                        <div class="col-md-6 mb-3">
                            <div class="p-3 border rounded">
                                ...
                                <small class="text-muted"><?= htmlspecialchars($inst['sigle']) ?></small>
                                <div class="progress mt-2" style="height:8px;">
                                    <div class="progress-bar bg-<?= $inst['sigle'] === 'ISTPK' ? 'primary' : 'info' ?>" 
                                         style="width:<?= min(100, $inst['count'] * 10) ?>%"></div>
                                </div>
                            </div>
                        </div>
                        <?php endforeach; ?>
                                ...
                                <tbody>
                                    <?php foreach ($etudiantsRecents as $etudiant) : ?>
                                        <tr>
                                            ...
                                            <td><span class="badge bg-secondary"><?= htmlspecialchars($etudiant['semestre']) ?></span></td>
